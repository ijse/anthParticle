...
Emitter.prototype.fire = function() {
    ...
    if(this.width) {
        model.oriPosition.x = util.getRandom(this.position.x, this.width);
    }
    if(this.height) {
        model.oriPosition.y = util.getRandom(this.position.y, this.height);
    }

    model.position = model.oriPosition.copy();
